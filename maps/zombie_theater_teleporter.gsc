...
teleporter_init()
{
	...
	//level.teleport_ae_funcs[level.teleport_ae_funcs.size] = maps\zombie_theater_teleporter::teleport_aftereffect_red_vision; //remove this loses fov
	//level.teleport_ae_funcs[level.teleport_ae_funcs.size] = maps\zombie_theater_teleporter::teleport_aftereffect_flashy_vision; // remove this teleports twice
...
player_teleporting( index )
{
	...
	if ( randomint( 100 ) > 50 && !IsDefined( level.eeroomsinuse ) ) // ww: chaning the frequency to 75%
	{
		loc = "eerooms";
		level.eeroomsinuse = true;

		if( RandomInt( 100 ) > 25  ) // 30% chance a powerup will spawn
		{
